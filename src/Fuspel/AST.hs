...
instance Show Fuspel where
  show (Fuspel imports rules) = (unlines . map show) imports ++ "\n" ++ showRules rules ++ "\n"
    where
      showRules = intercalate "\n\n" . map (intercalate "\n" . map show) . groupBy nameEq
        where
...
instance Show SimpleExpression where
  show (SEInt i) = show i
  show (SEName s) = s
  show (SEList l Nothing) = show l
  show (SEList l (Just t)) = "[" ++ (intercalate "," . map show) l ++ ":" ++ show t ++ "]"
...
instance Show Expression where
  ...
  show (EList l (Just t)) = "[" ++ (intercalate "," . map show) l ++ ":" ++ show t ++ "]"
